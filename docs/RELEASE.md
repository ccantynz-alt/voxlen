...
This document covers cutting a Marco Reid Voice release and the one-time setup for the auto-updater signing key.
...
```bash
npm run tauri signer generate -- -w ~/.tauri/marcoreid-voice.key
```

This produces:

- `~/.tauri/marcoreid-voice.key`        — **private** key (protect this; never commit)
- `~/.tauri/marcoreid-voice.key.pub`    — **public** key (paste into `tauri.conf.json`)
...
"updater": {
  "active": true,
  "endpoints": [
    "https://releases.marcoreid.com/{{target}}/{{current_version}}"
  ],
  "dialog": true,
...
> The `releases.marcoreid.com` host is a placeholder. Replace with your actual release server before shipping.
...
| Secret name           | Value                                                          |
| --------------------- | -------------------------------------------------------------- |
| `TAURI_PRIVATE_KEY`   | Contents of `~/.tauri/marcoreid-voice.key` (base64 or raw text)         |
...
5. The `Build & Release Marco Reid Voice` workflow will:
   - Build for macOS (aarch64 + x86_64), Windows, and Linux.
   - Attach signed bundles to a draft GitHub Release named `Marco Reid Voice vX.Y.Z`.
